...
## RPS
### Средний RPS:
Предположим, что каждый пользователь в среднем совершает:
- 5 запросов к API со статикой (главная, поиск, подборки, профиль, превью)
- 10 динамических запросов (лайки, паузы, получение рекомендаций и т.д.)

Итого 15 запросов на пользователя в день.

**RPS** = (51.2 млн × 15) / 86 400 = 768 млн / 86 400 ≈ 8 889 RPS

### Пиковый RPS:
В пиковый час (35.84 млн пользователей) предположим, что каждый совершает по 5 запросов к API.

**Пиковый RPS** = (35.84 млн × 5) / 3 600 = 179.2 млн / 3 600 ≈ 49 778 RPS
